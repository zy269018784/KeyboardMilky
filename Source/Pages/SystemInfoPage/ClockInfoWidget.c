...
void CreateClockInfoWidget(ClockInfoWidget* Widget, lv_obj_t* Parent, int x, int y, int w, int h)
{
	...
	Widget->Temp1 = CreateLabel(Widget->Handle, 0, 0, 150, h / 2, "5500MHz", lv_color_hex3(0xF00));
	lv_obj_add_style(Widget->Temp1, &style_label, 0);

	Widget->Temp2 = CreateLabel(Widget->Handle, 160, 0, 150, h / 2, "Clock", lv_color_hex3(0xF00));
